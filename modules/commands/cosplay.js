...
module.exports.run = async ({ api, event, Currencies }) => {
	...
	if (money >= 0) {
		...
		var callback = function () {
				...
				request(res.data.data).pipe(fs.createWriteStream(__dirname + '/cache/cosplay.jpg')).on("close", callback).then(Currencies.setData(event.senderID, options = {money: money - 0}));
			})
